...
from sklearn.metrics import confusion_matrix, f1_score, roc_auc_score, precision_score, recall_score, roc_curve, mean_absolute_error,mean_absolute_percentage_error, root_mean_squared_error,r2_score
import pandas as pd
...
def calculate_regressor_metrics(y_test, y_pred):
...
def evaluate_Regressor(y_test, y_pred, model_name="Model", enable_plot=True):

    # Calculate all metrics
    metrics = calculate_regressor_metrics(y_test, y_pred)

    if enable_plot:
        # Print detailed report
        p.print_regressor_report(metrics, model_name)
...
def train_evaluate_model(model, model_name, X_train, y_train, X_test, y_test, seed=None):
    ...
    return metrics
def train_evaluate_model_reg(model, model_name, X_train, y_train, X_test, y_test, seed=None):
    # Set random seed if provided and model has the parameter
    if seed is not None:
        if hasattr(model, 'random_state'):
            model.set_params(random_state=seed)
...
def train_evaluate_model_cv(model, model_name, X, y,
    ...
    return metrics
def train_evaluate_model_cv_reg(model, model_name, X, y,
                            preprocessor=None, cv=5, seed=None):
    ...
    return metrics
def train_evaluate_models_cv_reg(models: list, X, y, preprocessor=None, cv=5, seed=None):
    ...
    for model_name, model in models:
        ...
        # Store metrics
        all_metrics[model_name] = train_evaluate_model_cv_reg(
...
def train_evaluate_models_reg(models: list, X_train, y_train, X_test, y_test, seed=None):
    ...
    for model_name, model in models:
        ...
        # Store metrics
        all_metrics[model_name] = train_evaluate_model_reg(
            current_model, model_name, X_train, y_train, X_test, y_test, seed)
